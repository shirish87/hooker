...
function Hooker(options, callback) {
    ...
    if (!(this instanceof Hooker)) return new Hooker(options, callback);
...
Hooker.prototype.isDone = function () {
    ...
    this._browseHooks(function (err, hookData, event, i, total) {
...
Hooker.prototype.invoke = function () {
    ...
    this._browseHooks(event, function (err, hookData, event, i) {
        if (err) {
            this._raiseError('Hook not registered: ' + event);
