...
Hooker.prototype._createTrackCallback = function (hookData) {
    ...
    return function done(err) {
        var message = hookData.event + ' | ' + (hookData.name || 'unnamed') + ' (' + hookData.id + ')';
        debug('execution complete: ' + message);
...
Hooker.prototype.invoke = function () {
    ...
    this._browseHooks(event, function (err, hookData) {
        ...
        if (hookData.isDone || !hookData.callback) {
            // skip, since this has already been completed or is in progress
            return true;
        }

        var isTracked = false;

        // copy over original args for hook
        fnArgs = args.slice();

        if (hookData.waitTimeout) {
            fnArgs.push(self._createWaitCallback(hookData));
            isTracked = true;
        } else if (hookData.track) {
            fnArgs.push(self._createTrackCallback(hookData));
            isTracked = true;
        }

        // synchronous call
        debug('executing callback');
        hookData.callback.apply(self, fnArgs);

        // ensure hook callback is never called again
